...
export default function ProjectPage({ title, type, year, description, images, videos }) {
  ...
  return (
    <div className="flex w-full justify-center px-6 py-12 text-neutral-600 tracking-wide">
      <div className="relative mx-auto w-full max-w-screen-xl">
        ...
        <div className="mx-auto flex w-full max-w-[700px] flex-col items-start text-left">
          ...
          <h1 className="text-4xl font-semibold text-neutral-950 mb-2">{title}</h1>
          <p className="text-neutral-600 mb-8">{type} · {year}</p>
          ...
          {images.map((block, idx) => (
            <div key={idx} className="mt-10 flex w-full flex-col items-center">
              <div className="w-full md:w-[min(144%,calc(100vw-3rem))]">
                {Array.isArray(block.imgs) && block.imgs.map((img, i) => (
                  <div key={i} className="w-full mb-4">
                    <Image
                      ...
                      width={0}
                      height={0}
                      sizes="(min-width: 768px) 1008px, calc(100vw - 3rem)"
                      className="w-full h-auto rounded-2xl"
                      priority={idx === 0}
                    />
                  </div>
